	...
	if(_total_allocated + text_size + data_size > _max_size || text_size > _max_size || data_size > _max_size)
	{
		...
		if( _processes[index]->variables[0]->name.compare("<FREE_SPACE>")==0 )
		{
			...
			for(i=_processes[index]->_next_page; i<=number_of_pages; i++)
			{
				...
				var->size = _max_size ;//max available size
...
int Mmu::allocate( uint32_t pid, const std::string& var_name, const std::string& data_type, int num_element, PageTable *pageTable)
{
	...
	std::string data_type2 = data_type;
	int data_size = getDataSize( data_type2 );

	if( _max_size < data_size * num_element ){ std::cout << "Not enough space to allocate. \n"; return -1; }
...
int Mmu::free(uint32_t pid, std::string& var_name, PageTable *pageTable, uint8_t *memory)
{
	...
	for(int j=0; j<_processes[index]->variables[var_index]->page_numbers.size(); j++)
	{
		...
		name = _processes[index]->variables[var_index]->name;
		//kill all not shatred entries.
		if( !isShared(index, page_number, name) ){ pageTable->eraseEntry( pid, page_number ); }
		//_processes[index]->_next_page--;
	}
	...
	if(_processes[index]->variables.size() > var_index-1) 
	{
		...
		{
			_processes[index]->variables[var_index]->size +=  _processes[index]->variables[var_index+1]->size;
	...
	if(var_index >= 1)
	{
		if(_processes[index]->variables[var_index-1]->name == "<FREE_SPACE>")
		{
			_processes[index]->variables[var_index-1]->size +=  _processes[index]->variables[var_index]->size;
...
int Mmu::terminate(uint32_t pid, PageTable *pageTable, uint8_t *memory)
{
	int pid_index = findProcess(pid);
	if( pid_index == -1 ){ return -1; }//error

	int i, j, k;
	int page_number = 0, var_index=0, varAddr=0, physAddr=0, data_size=0, num_of_elements=0;
	std::string name;
	for(j=0; j<_processes[pid_index]->variables.size(); j++)
	{
		for(k=0; k<_processes[pid_index]->variables[j]->page_numbers.size(); k++)
		{
			page_number = _processes[pid_index]->variables[j]->page_numbers[k];
			name = _processes[pid_index]->variables[j]->name;
			pageTable->eraseEntry( pid, page_number );//kill all entries in the process.

			//reset memory
			var_index = findVariableIndex(name, pid_index);
			varAddr = findVariableAddr(name, pid_index);
			physAddr = pageTable->getPhysicalAddress(pid, varAddr);
			data_size = getDataSize( _processes[pid_index]->variables[var_index]->data_type );
			num_of_elements = _processes[pid_index]->variables[var_index]->size / data_size;

			for(int offset=0; offset<num_of_elements; offset++){ memory[physAddr+offset*data_size] = 0; }//reset values
		}
	}
	for( auto v : _processes[pid_index]->variables )
	{
		delete v;
	}
	_processes[pid_index]->variables.clear();//clear all varialbes
	_max_size = _max_size + _processes[pid_index]->p_virtual_address;//restore it to the size.
...
bool Mmu::isShared(uint32_t pid_index, int page_number, std::string& var_name)
{
	int j, k;
	...
	for(j = 0; j < _processes[pid_index]->variables.size(); j++)
	{
		for(k=0; k<_processes[pid_index]->variables[j]->page_numbers.size(); k++)
		{
			if( _processes[pid_index]->variables[j]->page_numbers[k] == page_number &&
				_processes[pid_index]->variables[j]->name !=  var_name )
			{
				return true;
			}
		}
	}
	return false;//pid == _processes[i]->pid
}
